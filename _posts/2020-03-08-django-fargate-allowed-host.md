...
왜 컨테이너를 사용할떄는 ALLOWED HOST에 IP 주소를 추가해야 할까? ECS 에이전트는 작업정의의 컨테이너에서 각 작업을 위한 '멈춤' 상태의 컨테이너를 만든다. 그러고 난 후 '멈춤' 상태의 컨테이너를 위해서 네트워크 네임스페이스를 세팅하고, 작업 내 다른 컨테이너도 시작해 멈춘 상태의 컨테이너와 같은 네트워크를 공유하게 만든다. 그래서 멈춘 컨테이너에 IPv4 주소를 추가만 해주면 문제가 해결되는 것이다. 
